...
const update = async () => {
    ...
    for(let i=0; i<countries.length; i++){
        ...
        const [max_date] = await pool.query('SELECT max(date) as max_date FROM rankings_'+ countries[i].iso_code)
        const max = new Date(max_date[0].max_date)
        console.log(last_date <= max, last_date, max)
        if (last_date <= max){
            while(last_date <= today){

                const [dia] = await pool.query('SELECT date FROM rankings_'+ countries[i].iso_code +' WHERE date = "'+ last_date.toISOString().split('T')[0] +'" LIMIT 1')
                console.log(dia)
